...
void print_stations_in_series(map<string, int> &metro) {
    cout << "\n\n---------- METRO STATIONS IN SERIES ---------" << endl;

    // Create a vector of pairs to store station names and their numerical representation
    vector<pair<string, int>> stations;

    // Populate the vector with station information
    for (auto i : metro) {
        stations.push_back({i.first, i.second});
    }

    // Sort the vector based on the numerical representation
    sort(stations.begin(), stations.end(), [](const auto &a, const auto &b) {
        return a.second < b.second;
    });

    // Print the stations in series
...
int main() {
    ...
    do {
        ...
        if (response == "1") {
            ...
            print_stations_in_series(metro);
        } else {
